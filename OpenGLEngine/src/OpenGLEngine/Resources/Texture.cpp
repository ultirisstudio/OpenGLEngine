...
namespace OpenGLEngine
{
	...
	Texture::Texture(const std::vector<unsigned char>& imageData, bool gamma = false, unsigned int min_filter_param = GL_NEAREST_MIPMAP_LINEAR, unsigned int mag_filter_param = GL_NEAREST) :
		m_id(0),
		m_Width(0),
		m_Height(0),
		m_Format(0),
		...
		m_path("")
	{
		...
		int width, height, nbChannels;
		unsigned char* data = stbi_load_from_memory(imageData.data(), imageData.size(), &width, &height, &nbChannels, 0);
	...
	std::shared_ptr<Texture> Texture::CreateTexture(const std::vector<unsigned char>& imageData, bool gamma)
	{
		return std::make_shared<Texture>(imageData, gamma);
	}
}